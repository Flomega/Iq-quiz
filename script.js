...
function createConfetti() {
    ...
    setTimeout(() => {
        container.remove();
    }, 5000); // Adjust this to match the longest animation duration
}
...
function showResult() {
    ...
    const iqEstimate = Math.min(160, 100 + (score / selectedQuestions.length) * 50); // Rough IQ estimate
    resultDiv.innerHTML = `Your final score is: ${score} out of ${selectedQuestions.length}<br>Your estimated IQ is: ${Math.round(iqEstimate)}`;
}
